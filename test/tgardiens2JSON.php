...
$stringOut .=$premierMatch;
$stringOut .=" au ";
$stringOut .=$dernierMatch;
//$ligueId = $lSaison['ligueRef'];
//$ligueId = 1;
}
...
while($lMatchs=mysql_fetch_array($rMatchs))  /// On regarde tous les matchs de la saison.
	{
					...
					$butsAccorde[$Im][0]=$lMatchs['score_dom'];
					$butsAccorde[$Im][1]=$lMatchs['eq_vis'];
...
$NbEntre=$Im;//count($butsAccorde);
	...
	{
		$stringOut.= "Début de cycle: ".$butsAccorde[$Ievent][3]." ";
		$stringOut.= "No: ".$butsAccorde[$Ievent][5]." ";
...
//	$NbRangeeStats=max(array_map('count',$rangeeStats));
	...
	$NbRangeeStats=count($rangeeStats);
		$stringOut.= "  ----  Nb gardien a afficher: ".$NbRangeeStats;
		$stringOut.= "  ----  Nb evenement a tester: ".$NbEntre;
	...
	while($Ievent<$NbEntre)  //$NbEntre=count($butsAccorde); Tous les �v�nements goaler
	{
			...
			while($indexJoueur<$NbRangeeStats)
				{
	$stringOut.= "A - ".$rangeeStats[$indexJoueur][0];
		$stringOut.= "B - ".$butsAccorde[$Ievent][3];
		...
		$stringOut.="  -  Test réussi!  : ".$butsAccorde[$Ievent][4]." - ".$indexJoueur." - ".$rangeeStats[$indexJoueur][0];
...
$stats=array();
	...
	while($Ievent<$NbRangeeStats)
		{
					$stringOut.=" dans JSON: ".$Ievent." - ".$rangeeStats[$Ievent][0];
			
$stats[$Ievent]['nom'] = $rangeeStats[$Ievent][0];
					$stringOut.=" dans JSON 2: ".$Ievent." - ".$stats[$Ievent]['nom'];
