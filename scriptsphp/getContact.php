...
//////////////////////////////////////////////////////
//
//  	1 - Obtenir la liste des ligue dont l'utilisateur est abonné.
//		2 - Obtenir le joueur associé
//		3 - Obtenir les ligues et équipes et dont le joueur est abonné
//		4 - Obtenir les profils d'arbitres
//
//
//
//
...
$userId = trouveIDParNomUser($username);
...
for($a=0;$a<count($listeLigue);$a++){
	...
	$resCon = mysql_query("SELECT TableUser.prenom,TableUser.nom,TableUser.username,TableUser.noCompte 
						FROM  abonJoueurLigue
						JOIN TableJoueur
							ON (abonJoueurLigue.joueurId=TableJoueur.joueur_id)
						JOIN TableUser
							ON (TableJoueur.proprio=TableUser.noCompte)
							
						WHERE ligueId='{$listeLigue[$a]}'") or die(mysql_error());
	$liste[$a]['ligueId'];
	while($rangCon=mysql_fetch_assoc($resCon)){
		if(!in_array($rangCon,$contact)){
		array_push($contact,$rangCon);}
	}
...
for($a=0;$a<count($listeEquipe);$a++){
	...
	$resCon = mysql_query("SELECT TableUser.prenom,TableUser.nom,TableUser.username,TableUser.noCompte
						...
						WHERE equipeId='{$listeEquipe[$a]}'") or die(mysql_error());
	while($rangCon=mysql_fetch_assoc($resCon)){
		if(!in_array($rangCon,$contact)){
		array_push($contact,$rangCon);}
	}}
...
//$JSONstring .="{\"contacts\": ".json_encode($contact). ",\"joueur\": ".json_encode($joueur). ",\"arbitre\": ".json_encode($arbitre)
