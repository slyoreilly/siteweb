...
$arrRecept = $request->recepteur;
$expediteur = trouveIDParNomUser($request->expediteur);
$titre = mysql_real_escape_string($request->titre);
$corps = mysql_real_escape_string($request->corps);

$IR=0;
while($IR<count($arrRecept))
{
$recepteur=$arrRecept[$IR];
$contexte = '{"medium":"marche"}';
	$retour = mysql_query("INSERT INTO TableMessage (expediteur, recepteur, titre, corps, dateEmission,cleValeur) 
VALUES ('{$expediteur}','{$recepteur}','{$titre}','{$corps}',NOW(),'{$contexte}')")or die(mysql_error()." INSERT INTO");
...
$reponse="\n\n"."Pour répondre à ce courriel, suivez ce lien: "."http://www.syncstats.com/zadmin/messages.html?messageId=".$retour."&mode=reply";
 ...
 $to = $courriel;
 $subject = $titre;
 $body = "Expéditeur: ".$_POST['expediteur']. "\n\n"."Message: ".$corps.$reponse;
