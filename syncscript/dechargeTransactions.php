...
$retour = mysqli_query($conn,"INSERT INTO TableJoueur (NomJoueur, NumeroJoueur, equipe_id_ref,Equipe, Ligue, ficIdPortrait,nom,prenom,villeOrigine,taille,poids,anneeNaissance,ficIdJoueur,proprio) 
VALUES ('{$intJoueur}', '{$intNo}', NULL,'aucune', NULL,95,'','','',0,0,2000,95,0)")or die(mysqli_error($conn)."insert bug");	
...
//VALUES ( 'test	Match2', 'testMatch2', 'testMatch2', 'testMatch2','testMatch2')");	
	...
	$resultNouveau = mysqli_query($conn,"SELECT joueur_id FROM TableJoueur WHERE Nomjoueur='{$intJoueur}' AND NumeroJoueur='{$intNo}' ORDER BY joueur_id DESC")
				or die(mysqli_error($conn)."select bug");  
