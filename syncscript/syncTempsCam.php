...
$qLigues = "SELECT ID_Ligue, nomLigue,  FROM Ligue
			INNER JOIN AbonnementLigue
...
while($r = mysqli_fetch_array($resultLigues)) {
	...
	$vecLigues[$IL2]['cleValeur']=$r['cleValeur'];
    $IL2++;
    }
...
while ($rangeeMatch = mysqli_fetch_array($resultMatch)){// && !$trouve) {
	...
	{		$qSelEqDom="SELECT * From TableEquipe
		...
		while($rangDom = mysqli_fetch_array($resEqDom)){
			$nomEqDom =$rangDom['nom_equipe'];
		}				
	...
	{		$qSelEqVis="SELECT * From TableEquipe
		...
		while($rangVis = mysqli_fetch_array($resEqVis)){
			$nomEqVis =$rangVis['nom_equipe'];
		}						
	...
	if(!in_array($rangeeMatch['match_id'], $matchs))
	{
		...
		while($rangAr = mysqli_fetch_array($resArena)){
			$nomArena=$rangAr['nomArena'];
			$nomGlace=$rangAr['nomGlace'];
		}	
		...
		$matchs[$IM2]['date']=$rangeeMatch['date'];
				$IM2++;
					...
					$qChron = "SELECT event_id,chrono,match_event_id,equipe_event_id,TableMatch.match_id,TableMatch.ligueRef,TableMatch.eq_dom,TableMatch.eq_vis FROM TableEvenement0 
							INNER JOIN TableMatch
								ON (TableEvenement0.match_event_id=TableMatch.matchIdRef)
