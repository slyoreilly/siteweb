...
error_log("▶️ Début script optimisé avec JOIN");
...
if (!$ligueId || !$plateauId) {
    error_log("❌ Paramètres GET manquants");
...
if ($stmt = mysqli_prepare($conn, $sql)) {
    ...
    error_log("✅ Requête JOIN exécutée, événements total : " . count($resultats));
} else {
    error_log("❌ Échec préparation requête JOIN : " . mysqli_error($conn));
    die(json_encode(["error" => "Erreur SQL."]));
}

mysqli_close($conn);
echo json_encode(array_values($resultats)); // array_values pour obtenir un tableau indexé
?>
